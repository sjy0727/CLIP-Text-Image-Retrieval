...
class HfVisionModel:
    ...
    def _load_model_and_processor(self, model_name):
        model = CLIPVisionModelWithProjection.from_pretrained(model_name).to(self.device)
        processor = AutoProcessor.from_pretrained(model_name)
        return model, processor

    def _warmup(self):
        input = self.processor(images=Image.open('./assets/demo.png'), return_tensors='pt', padding=True).to(self.device)
        self.model.eval()
        with torch.no_grad():
            self.model(**input)
    ...
    def __call__(self, images):
        image_token = self.processor(images=images, return_tensors='pt', padding=True).to(self.device)
        self.model.eval()
        with torch.no_grad():
